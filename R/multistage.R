...
svydesign.default <- function(ids, probs = NULL, strata = NULL, variables = NULL, fpc = NULL,
  ...
  if (is.character(pps)) {
    ...
    if (!(pps %in% c("brewer", "other"))) {
      return(pps_design(
        ids = ids, probs = probs, strata = strata, variables = variables, fpc = fpc,
    ...
    v.sub <- by(1:n, list(as.numeric(clusters[, 1])), function(index) {
      ...
      for (cali in cal) {
        ...
        if (length(unique(j)) != 1) {
          stop(
            "Internal problem in g-calibration data: stage", stage,
            ", cluster", j
          )
        }
        j <- j[[1]]
      ...
      multistage(x[index, , drop = FALSE], clusters[index, -1, drop = FALSE],
        ...
        stage = stage + 1, cal = cal
      ) * nPSUs[index[1], 1] / fpcs[index[1], 1]
    })
...
as.fpc <- function(df, strata, ids, pps = FALSE) {
  ...
  if (any(popsize < sampsize)) {
    ...
    cat(
      "record", toobig[1, 1], "stage", toobig[1, 2], ": popsize=", popsize[toobig[1, , drop = FALSE]],
    ...
    if (typeof(x) %in% c("expression", "symbol")) {
      ...
      if (is.data.frame(x) && any(sapply(x, is.factor))) {
        xx <- lapply(x, function(xi) {
          if (is.factor(xi)) 0 + (outer(xi, levels(xi), "==")) else xi
        })
        cols <- sapply(xx, NCOL)
        scols <- c(0, cumsum(cols))
...
svyratio.survey.design2 <- function(numerator = formula, denominator, design, separate = FALSE, na.rm = FALSE,
  ...
  if (covmat) {
    ...
    jj <- rep(1:nd, each = nn)
    allr <- sweep(
      numerator[, ii] - t(as.vector(rval$ratio) * t(denominator[, jj, drop = FALSE])),
